# clandro-create-package

A utility to create [binary deb packages](https://www.debian.org/doc/debian-policy/ch-binary.html).

By default it creates binary deb packages for installation in the [Clandro](https://clandro.com) Linux environment, but by passing the `--prefix /usr` argument or defining `installation_prefix: "/usr"` field in the `YAML` or `JSON`  manifest, a deb file can be created for linux distributions such as Debian or Ubuntu, etc.
##
...
- Android version `>= 7.0` using [Clandro App].
- Linux distros.
...
- [GitHub releases](https://github.com/cl-andro/clandro-create-package/releases).
##
...
Note that `clandro-create-package` is no longer updated on the [`Python Package Index (PyPI)`](https://pypi.org/project/clandro-create-package/) and should not be installed with `pip`. Latest version on `PyPi` is `0.7`.
##
...
```
clandro-create-package command is used to create binary deb packages.

Usage:
  clandro-create-package [optional arguments] manifests...
...
optional arguments:
  ...
  --prefix PREFIX       path under which to install the files on the target system
                        (default: /data/data/com.clandro/files/usr,
...
control:
    ...
    Description: |-
        This is the hello world package
         It is just an example for clandro-create-package
         .
         It is just prints 'Hello world'

installation_prefix: /data/data/com.clandro/files/usr

data_files:
    bin/hello-world:
        source: hello-world.py
        set_shebang: "#!/data/data/com.clandro/files/usr/bin/env python3"
...
The prefix under which to install the files on the target system. The clandro prefix `/data/data/com.clandro/files/usr` is used by default if the field is not defined, unless `--prefix` argument is passed to the script. It must be an absolute path that starts with forward slash `/`, ends with `/usr` and must only contain characters in the range `a-zA-Z0-9_./`. It cannot contain parent path references `../`.


#### `maintainer_scripts_shebang`

The shebang that should be set on the maintainer scripts if the first line starts with `#!`. Example: `#!/bin/bash` for linux distros and `#!/data/data/com.clandro/files/usr/bin/bash` for clandro.
...
**Note that when installing the `deb` file on android with the clandro app, any custom ownership value will not be set and all files will be set to `clandro` user ownership.**
...
The shebang that should be set on the data file if the first line starts with `#!`. Example: `#!/bin/bash` for linux distros and `#!/data/data/com.clandro/files/usr/bin/bash` for clandro.
...
After creating a manifest for the project, run `clandro-create-package </path/to/manifest>` command to create the deb file. Example manifests are provided for both `YAML` and `JSON` in the [`examples`](examples) directory.

- Create a deb package file with defaults: `clandro-create-package manifest.yml`

- Create a deb package file with specific installation prefix, files directory, deb directory and deb name: `clandro-create-package --prefix '/usr' --files-dir '/path/to/files_directory' --deb-dir '/path/to/deb_directory' --deb-name 'some-name.deb' manifest.json `

- Create example manifest deb from `clandro-create-package` repo source: `cd examples/hello-world; ../../src/clandro-create-package -vv manifest-ubuntu.yml`
...
The deb file can also be added to a custom apt repository created with [`clandro-apt-repo`](https://github.com/cl-andro/clandro-apt-repo) or any other available tool.
##



[Clandro App]: https://github.com/cl-andro/clandro-app
